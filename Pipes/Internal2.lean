...
-- Explicit bind function for Proxy

-- @[inline, simp]
-- def Proxy.bind {a' a b' b : Type u} {m : Type u → Type u} [Functor m]
--   (proxy : Proxy a' a b' b m r) (f : r → Proxy a' a b' b m s) : Proxy a' a b' b m s :=
--   F.bind proxy f
-- @[inline] instance [Functor m] : Bind (Proxy a' a b' b m) := ⟨Proxy.bind⟩
...
@[inline] def Proxy.runProxy [Monad m] [Functor m] (p : Proxy Empty any any2 Empty m r) : m r :=
  p.iterM fun
...
namespace AlternativeTest
  ...
  #guard testAlt1.runProxy = .none
  #guard testAlt2.runProxy = .some "world"

  #guard (testAlt1 <|> testAlt2).runProxy = testAlt1.runProxy -- but should be 2
end AlternativeTest
...
| []      => pure ()
| (a::as) => Proxy.respond a *> each as

def Proxy.Array.each [Monad m] [Functor m] : (Array a) -> Producer_ a m Unit := (Proxy.List.each ·.toList)

-- Run an Effect computation
@[inline] abbrev runEffect [Monad m] [Functor m] : Effect m r -> m r := Proxy.runProxy

-- Flipped version of composeResponse
@[inline] def flipComposeResponse [Monad m] [Functor m]
    (fb : b → Proxy x' x c' c m b')
    (p0 :     Proxy x' x b' b m a') :
              Proxy x' x c' c m a' :=
  ⟨fun kp kf => p0.runF kp fun
    | ProxyF.Request x' fx => kf (ProxyF.Request x' (flipComposeResponse fb ∘ fx))
    | ProxyF.Respond b fb' => (fb b >>= fun b => flipComposeResponse fb $ Pure.pure $ fb' b).runF kp kf
    | ProxyF.M m => kf (ProxyF.M (flipComposeResponse fb <$> m))⟩
